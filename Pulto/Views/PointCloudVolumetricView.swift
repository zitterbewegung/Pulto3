...
struct Model3DVolumetricView: View {
    ...
    private func create3DModelEntity(from data: Model3DData) async -> Entity {
        ...
        if !positions.isEmpty {
            ...
            do {
                // Generate mesh resource from descriptor
                let meshes = try MeshResource.generate(from: [meshDescriptor])
                let mesh   = meshes
                ...
                if wireframeMode {
                    ...
                    //simpleMaterial.color = .init(tint: Color(hex: selectedMat.color)
                    //simpleMaterial.roughness = .float(Float(selectedMat.roughness ?? 0.5))
