...
struct EntryPoint: App {
    ...
    private var mainWindow: some SwiftUI.Scene {
        WindowGroup(id: "main", for: Int.self) { $id in
            if let id = id {
                NewWindow(id: id)
    ...
    private var volumetricWindows: some SwiftUI.Scene {
        ...
        WindowGroup(id: "volumetric-pointcloud", for: Int.self) { $id in
            if let id = id {
                ...
                if let win = windowManager.getWindow(for: id) {
        ...
        WindowGroup(id: "volumetric-chart3d", for: Int.self) { $id in
         ...
         CdataURL: <#URL?#>, hart3DView(windowID: id, chartData: window.state.chart3DData)
                        .glassBackgroundEffect()
                }
            }
        }
        ...
        // COMMENTED OUT: Spatial Editor and Metrics windows
        /*
        WindowGroup(id: "spatial-editor", for: Int.self) { $id in
            if let id = id {
        ...
        WindowGroup(id: "metrics-viewer", for: Int.self) { $id in
            if let id = id {
                MetricsViewer(windowID: id)
                    .glassBackgroundEffect()
            }
        }
        */
         /*var immersiveWorkspace: some SwiftUI.Scene {
